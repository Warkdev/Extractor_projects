...
class ModelFile {
public:
	ModelFile(std::string directory, std::string hash, std::string filename);
	~ModelFile();
	...
	struct VmapGroup {
		...
		struct {
			...
			unsigned int size = 0;			// sizeof(MLIQ::Header) + (sizeof(LiquidVert) * liquidInfo->header.xVerts * liquidInfo->header.yVerts) + (liquidInfo->header.xTiles * liquidInfo->header.yTiles);
...
private:
	const std::string PATTERN_FILE = "%s-%s"; // hash-filename (including .wmo)
